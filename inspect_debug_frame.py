...
if saved_frame.ndim == 3 and saved_frame.shape[2] == 3:
    ...
    saved_frame = cv2.cvtColor(saved_frame, cv2.COLOR_GRAY2RGB)

# Check the frame again after conversion
print(f"Converted frame shape: {saved_frame.shape}, dtype: {saved_frame.dtype}")
print(f"Converted frame min value: {saved_frame.min()}")
print(f"Converted frame max value: {saved_frame.max()}")

# Try to use the face_recognition library on the loaded frame
try:
    face_locations = face_recognition.face_locations(saved_frame, model="cnn")
...
try:
    face_locations = face_recognition.face_locations(saved_frame, model="hog")
